...
int time_enc16(double *av, double *sig, unsigned int k_len, unsigned long long dataLengthInBytes, unsigned long long AdataLengthInBytes)
{
	...
	while(!sam_cnt)
	{
		...
		{
			cy = (double)read_tsc();
			ThetaCB3_enc(pt[0], tag, nonce, at[0], pt[0], AdataLengthInBytes, dataLengthInBytes, key);
			ThetaCB3_enc(pt[1], tag, nonce, at[1], pt[1], AdataLengthInBytes, dataLengthInBytes, key);
			ThetaCB3_enc(pt[2], tag, nonce, at[2], pt[2], AdataLengthInBytes, dataLengthInBytes, key);
			ThetaCB3_enc(pt[3], tag, nonce, at[3], pt[3], AdataLengthInBytes, dataLengthInBytes, key);
			ThetaCB3_enc(pt[0], tag, nonce, at[0], pt[0], AdataLengthInBytes, dataLengthInBytes, key);
			ThetaCB3_enc(pt[1], tag, nonce, at[1], pt[1], AdataLengthInBytes, dataLengthInBytes, key);
			ThetaCB3_enc(pt[2], tag, nonce, at[2], pt[2], AdataLengthInBytes, dataLengthInBytes, key);
			ThetaCB3_enc(pt[3], tag, nonce, at[3], pt[3], AdataLengthInBytes, dataLengthInBytes, key);
...
double enc_cycles(unsigned int k_len, unsigned long long dataLengthInBytes, unsigned long long AdataLengthInBytes)
{
	...
	ThetaCB3_enc(pt, tag, nonce, at, pt, AdataLengthInBytes, dataLengthInBytes, key);
...
void timing()
{
	...
	fn = "ThetaCB3_ENC_Timing_PIPE"+ to_string((u64)PIPE) + ".csv";
	...
	fn = "ThetaCB3_ENC_Short_Timing_PIPE"+ to_string((u64)PIPE) + ".csv";
	fout.open(fn.c_str(), ios::app);
	fout.setf(ios::fixed);
	fout << std::right;
